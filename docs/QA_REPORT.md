...
## 2026-08-06 수장고 미복원 작품 우선 정렬

- 수장고에서 검색어·재료·복원 상태 필터를 적용한 뒤 미복원 작품을 먼저, 복원 완료 작품을 가장 아래에 안정 정렬한다.
- 작품 더 보기의 24개 단위 분할보다 먼저 정렬하므로, 모바일과 데스크톱 모두 첫 목록에서 새 복원 후보를 우선 확인할 수 있다.
- `복원 완료` 필터와 기록실의 작품 기록·재전시 기능은 그대로 유지하며 작품 데이터와 세이브 순서는 바꾸지 않는다.
- 전용 정렬 검사에서 원본 배열 불변, 미복원·복원 내부 순서 유지, 필터 뒤 정렬을 확인했다.
- 복원 완료 작품 1점이 있는 저장으로 390×844와 1280×900 수장고를 열어 첫 24개에 완료 카드가 없고, 첫 카드가 다음 미복원 작품으로 바뀌며, `복원 완료` 필터에서는 해당 완료 작품이 다시 나타나는 것을 확인했다.
