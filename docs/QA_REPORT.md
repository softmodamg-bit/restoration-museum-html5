...
## 2026-08-08 PC·폴드·모바일 반응형 안정화

- 1400px 이하 또는 화면 높이 850px 이하에서는 튜토리얼 전체 카드를 작은 비서 안내 버튼으로 접어, 복원 작업 화면과 클릭 지점을 가리지 않게 했다. 펼친 설명 카드의 빈 영역은 포인터 입력을 가로채지 않고 실제 버튼만 조작된다.
- 1160px 이하 전시관에서는 `시설을 꾸미고 가꾸기`를 전시 장면보다 앞에 놓인 접이식 패널로 바꾸고, 펼친 목록은 패널 안에서만 스크롤하도록 했다. 폴드 중간 폭에서 두 열 중 한쪽이 빈 공간으로 남던 배치를 제거했다.
- 680px 이하 비서실은 `100dvh`와 네 방향 안전영역을 사용해 브라우저 주소창·하단 제스처 영역 안에 맞췄다. 패널 내용은 자체 스크롤되고 상단 얼굴·제목·닫기 버튼은 고정된다.
- 전체 랭킹은 데스크톱에서 새 탭을 유지하고, 모바일·터치 장치에서는 팝업 차단을 피하도록 같은 탭에서 연다. 랭킹 페이지의 돌아가기 버튼은 탭 닫기가 허용되지 않을 때 브라우저 뒤로 가기로 복귀한다.
- 실제 브라우저에서 1348×1012 복원실의 드러난 손상 지점을 클릭해 `1 / 3 기록 완료`로 진행되는 것을 확인했다. 390×700 비서실은 패널 높이 688px, 내부 스크롤 410px로 마지막 버튼까지 조작됐고, 868×766 전시관 시설 패널은 화면 위에서 열리고 내부 스크롤이 적용됐다.
- 390×700에서 Google Apps Script 전체 랭킹 화면이 같은 탭으로 실제 열렸고 순위 목록이 표시됐다. 1348×1012에서는 링크가 `_blank`로 유지되는 것을 확인했다.
- 320·360·390·600·868·1024·1348·1600px 폭을 교차 측정한 결과 페이지 가로 넘침은 모두 0이었다. `node --check js/game.js`와 전체 `scripts/test-*.mjs`도 통과했다.

## 2026-08-08 모바일 길게 누르기 입력 보호

- 복원 작업판 안에서 길게 누를 때 브라우저의 컨텍스트 메뉴, 텍스트 선택 손잡이, 이미지 끌기가 나타나지 않도록 공용 입력 보호를 추가했다.
- 실제 홀드 게임인 정밀 처리량 조절 버튼은 `touch-action: none`과 iOS 터치 콜아웃 차단을 함께 적용해 누르는 동안 게임의 `pointerdown`·`pointerup` 판정이 유지된다.
- 차단 범위는 `.mechanic-layer` 안으로 한정했다. 비서실 이름 입력과 게임 밖 일반 문구의 브라우저 입력·선택 기능은 바꾸지 않았다.

## 2026-08-08 모바일 첫 관장 설정 스크롤

- 공통 스크롤 규칙을 `overflow: hidden`으로 덮어쓰던 첫 관장 설정 모달과 스토리 모달을 세로 스크롤 가능한 구조로 바로잡았다.
- 680px 이하에서는 동적 화면 높이(`100dvh`)와 위·아래 안전 영역을 반영해, 주소창이나 화면 키보드가 올라온 짧은 화면에서도 하단 확인 버튼까지 손가락으로 내릴 수 있다.
- 취임 설정을 다시 열거나 이름 확인 뒤 비서의 첫 인사 단계로 넘어갈 때 이전 스크롤 위치가 남지 않고 맨 위에서 시작한다.
- 실제 390×600 화면에서는 설정 모달이 `clientHeight 576 / scrollHeight 923`, 390×500에서는 `476 / 923`으로 계산됐다. 두 화면에서 손가락 방향 스크롤을 재현해 확인 버튼 전체를 화면 안으로 옮겨 클릭했으며, 다음 인사 단계의 `scrollTop`이 0으로 돌아오는 것과 가로 넘침·콘솔 오류가 없는 것을 확인했다.

## 2026-08-08 모바일 튜토리얼 자동 강조

- 접힌 모바일 튜토리얼에서 수동 `강조 위치 보기`를 누른 뒤에만 목표가 보이던 조건을 제거했다.
- 첫 운영 안내에 다시 들어오거나 단계가 바뀔 때는 비서 설명 카드를 펼치지 않아도 목표 위치로 자동 이동하고 강조 테두리와 짧은 어두운 배경을 바로 표시한다.
- 비서 버튼과 `강조 위치 보기`는 안내를 다시 읽거나 놓친 목표를 다시 확인하는 용도로 그대로 유지한다.
- 실제 390×700 화면에서 비서실을 열지 않은 채 프롤로그를 닫자 접힌 안내 상태(`aria-expanded=false`) 그대로 `작품 이야기` 버튼이 화면 안으로 이동해 강조 테두리와 어두운 배경이 즉시 나타났다. 작품 설명을 닫은 다음 단계에서도 `복원실로 옮기기` 버튼으로 자동 강조가 갱신됐고 가로 넘침·콘솔 오류는 없었다.
