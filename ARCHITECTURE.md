...
- 정적 HTML5 배포, 마우스·터치 지원, 기존 `localStorage` 세이브 호환성을 유지한다. GitHub Pages 게시 원본은 `GitHub Actions`이며, `.github/workflows/pages.yml` 한 곳에서만 `index.html`, `styles.css`, `js/`, `assets/`를 `_site` 아티팩트로 올린다. 백업 ZIP·검수 문서·개발 스크립트는 게시물에서 제외해 기존 브랜치 자동 게시 작업과의 중복·불필요한 업로드를 막는다.
...
첫 운영 안내는 작업판을 가리지 않는 넓은 화면에서만 기존 안내 카드로 표시한다. 1,400px 이하 또는 화면 높이 850px 이하에서는 `#tutorialMobileToggle`의 원형 선택 비서 프로필로 접고, 사용자가 누를 때만 `#tutorialGuideCard`를 펼친다. 설명 카드 자체는 포인터 입력을 가로채지 않고 카드 안의 실제 버튼만 입력을 받으므로, 카드가 복원 작업판과 겹쳐도 미니게임 조작을 막지 않는다. 새 튜토리얼 단계가 시작되면 접힌 모바일에서도 목표 위치로 자동 이동해 강조 테두리와 짧은 어두운 배경 연출을 바로 보여 준다. 강조 테두리는 다음 목표로 넘어갈 때 함께 갱신되며, 놓쳤을 때는 `강조 위치 보기`로 같은 목표를 다시 확인할 수 있다.

전시관은 조작 UI와 장면의 반응형 전략을 분리한다. `#galleryDioramaViewport` 안의 `#galleryDiorama`만 680px 이하에서 760×670 기준 장면 전체를 폭에 맞춰 축소한다. 따라서 작품 수를 줄이거나 페이지 가로 스크롤을 만들지 않는다. 편의동(`.gallery-annex`)과 운영 패널은 축소하지 않고 정상 글자·터치 크기로 배치한다. 폴드·태블릿을 포함한 1,160px 이하에서는 `.management-panel`을 `display: contents`로 풀어 `.upgrade-panel-card`를 장면보다 먼저 배치하고, `#mobileUpgradeToggle`로 시설 목록을 접고 편다. 펼친 목록은 동적 뷰포트 높이 안에서 독립적으로 스크롤하며, 화면을 벗어나거나 넓은 화면으로 전환하면 `setMobileUpgradeOpen()`이 접힌 상태로 정리한다. 장면 배율은 `updateGalleryDioramaScale()`이 `ResizeObserver`와 화면 전환 뒤에 다시 계산한다.

비서실 `#assistantPanel`은 데스크톱에서도 뷰포트 안에서 세로 스크롤하며, 680px 이하에서는 `100dvh`와 네 방향 `safe-area-inset` 사이에 고정한다. 패널 머리글은 내부 스크롤에 붙어 있어 비서 초상화와 닫기 버튼이 항상 접근 가능하다. 브라우저 주소창이 나타나거나 사라져 실제 화면 높이가 바뀌어도 패널 아래 입력·저장 버튼이 잘리지 않아야 한다.

공용 `.modal-backdrop`과 모달 본문은 세로 `pan-y`를 허용한다. 특히 첫 관장 설정 `#directorModal`과 스토리 모달은 둥근 모서리를 위한 가로 자르기만 유지하고 세로 방향은 자체 스크롤한다. 680px 이하에서는 `100dvh`와 위·아래 `safe-area-inset`을 뺀 높이를 상한으로 사용하므로, 주소창이나 화면 키보드로 뷰포트가 짧아져도 이름·미술관 입력 뒤의 확인 버튼까지 손가락으로 내릴 수 있어야 한다. 취임 설정을 처음 열거나 비서 인사 단계로 전환할 때는 `scrollTop`을 0으로 되돌린다.
...
보존 연습 목록은 넓은 PC에서 2열을 유지하되 900px 이하의 폴드 펼침·태블릿 구간에서는 1열로 전환해 아이콘·설명·난이도 선택 영역이 서로 폭을 빼앗지 않게 한다. 680px 이하에서는 기존처럼 카드 동작 영역을 아래 행으로 내려 좁은 화면의 터치 크기를 확보한다. 카드의 한국어 제목과 설명은 `word-break: keep-all`을 사용해 글자가 한 음절씩 세로로 끊기지 않아야 한다.
...
- `전체 랭킹 확인`은 모바일 팝업 차단과 일부 모바일 브라우저의 `script.google.com` 파일 열기 충돌을 피하기 위해 `window.open()`이나 Apps Script 최상위 직접 이동을 쓰지 않는다. `renderRankingPanel()`은 같은 GitHub Pages 출처의 `ranking.html`을 `#viewRankingButton`의 실제 `href`에 넣는다. `ranking.html`이 `js/ranking-config.js`의 Apps Script 조회 주소를 검증한 뒤 전체 화면 `iframe` 안에 공개 랭킹을 표시하며, 직접 열기와 게임으로 돌아가기 대체 동선도 제공한다. Apps Script 안쪽의 돌아가기 버튼은 `RESTORATION_RANKING_RETURN` 메시지를 최상위 화면에 보내고, `ranking.html`은 Google Script 출처만 확인해 같은 `returnToGame()` 경로로 복귀한다. 랭킹 제출 직후 새 탭에는 큰 제목과 진행 표시가 있는 모바일 대응 로딩 카드를 먼저 렌더링한다. `.github/workflows/pages.yml`은 이 전용 화면도 Pages 산출물에 복사해야 한다. 마우스 중심의 넓은 화면은 일반 링크 클릭으로 새 탭을 열고, 820px 이하 또는 거친 포인터 터치 기기는 현재 탭에서 연다. 랭킹 주소가 비어 있을 때만 클릭 기본 동작을 막고 기존 미리보기 모달을 연다.
