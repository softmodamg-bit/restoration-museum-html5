...
- `전체 랭킹 확인`은 모바일 팝업 차단을 피하기 위해 `window.open()`을 쓰지 않는다. `renderRankingPanel()`이 Apps Script 조회 주소를 `#viewRankingButton`의 실제 `href`에 넣는다. 마우스 중심의 넓은 화면은 일반 링크 클릭으로 새 탭을 열고, 820px 이하 또는 거친 포인터 터치 기기는 현재 탭에서 직접 연다. Apps Script의 돌아가기 버튼은 새 탭이면 그 탭만 닫고, 모바일 현재 탭이면 브라우저 기록으로 게임에 돌아온다. 랭킹 주소가 비어 있을 때만 클릭 기본 동작을 막고 기존 미리보기 모달을 연다.
