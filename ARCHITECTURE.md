...
`index.html`의 `.sticky-navigation`은 상단 상태 바(`.topbar`)와 메인 메뉴(`.main-tabs`)를 한 묶음으로 고정한다. 스크롤 중에도 수장고·복원실·전시관·스토리·기록실·보존 연습 메뉴가 함께 남아야 하므로 두 요소를 다시 개별 `sticky`로 분리하지 않는다. 비서 안내와 모달은 이 영역보다 높은 레이어를 사용한다.
