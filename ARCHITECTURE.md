...
첫 운영 안내는 작업판을 가리지 않는 넓은 화면에서만 기존 안내 카드로 표시한다. 1,400px 이하 또는 화면 높이 850px 이하에서는 `#tutorialMobileToggle`의 원형 선택 비서 프로필로 접고, 사용자가 누를 때만 `#tutorialGuideCard`를 펼친다. 설명 카드 자체는 포인터 입력을 가로채지 않고 카드 안의 실제 버튼만 입력을 받으므로, 카드가 복원 작업판과 겹쳐도 미니게임 조작을 막지 않는다. 접힌 동안에는 강조 테두리와 어두운 배경도 숨기며, `강조 위치 보기`를 직접 누른 뒤에만 잠시 목표를 강조한다.
