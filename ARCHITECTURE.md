...
- 정적 HTML5 배포, 마우스·터치 지원, 기존 `localStorage` 세이브 호환성을 유지한다. GitHub Pages 게시 원본은 `GitHub Actions`이며, `.github/workflows/pages.yml` 한 곳에서만 정적 파일을 올려 기존 브랜치 자동 게시 작업과의 중복·충돌을 막는다.
