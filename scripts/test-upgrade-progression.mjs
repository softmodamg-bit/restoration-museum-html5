...
const baseUpgrades = upgrades.filter(upgrade => !upgrade.tier || upgrade.tier === 1);
const advancedUpgrades = upgrades.filter(upgrade => upgrade.tier === 2);
const finalUpgrades = upgrades.filter(upgrade => upgrade.tier === 3);
...
assert(finalUpgrades.length === 12, `3단계 시설은 12개여야 합니다: ${finalUpgrades.length}`);
assert(advancedUpgrades.every(upgrade => upgrade.unlockDay === 15), "모든 2단계 시설은 15일차에 열려야 합니다.");
assert(finalUpgrades.every(upgrade => upgrade.unlockDay === 25), "모든 3단계 시설은 25일차에 열려야 합니다.");
...
assert(finalUpgrades.every(upgrade => advancedUpgrades.some(item => item.id === upgrade.requires)), "3단계 시설의 선행 2단계 시설이 누락되었습니다.");
assert(new Set(finalUpgrades.map(upgrade => upgrade.requires)).size === advancedUpgrades.length, "각 2단계 시설에 정확히 한 개의 3단계 개선이 있어야 합니다.");
...
assert(finalUpgrades.reduce((sum, upgrade) => sum + upgrade.cost, 0) >= 100000, "25일차 이후 코인 소비 규모가 충분하지 않습니다.");
...
assert(gameSource.includes("baseFacilityIdForUpgrade(upgrade)"), "단계 완료 연출이 기본 시설을 가리키지 않습니다.");
