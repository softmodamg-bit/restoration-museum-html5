...
function closeRankingButton_() {
  return "<button type='button' onclick='closeRankingTab()'>랭킹 탭 닫고 게임으로 돌아가기</button>"
    ...
    + "<script>function closeRankingTab(){var canGoBack=window.history.length>1;try{window.top.postMessage({type:'RESTORATION_RANKING_RETURN'},'*');}catch(ignore){}setTimeout(function(){try{window.top.close();}catch(error){try{window.close();}catch(ignore){}}setTimeout(function(){if(!document.hidden&&canGoBack){window.history.back();return;}var hint=document.getElementById('closeHint');if(hint)hint.style.display='block';},280);},80);}<\/script>";
}
